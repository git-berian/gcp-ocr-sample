...
- **コスト**: 既定モデル `claude-opus-4-8` は **入力 $5.00 / 出力 $25.00（100万トークンあたり、2026-06 時点の一覧価格）** で、Gemini 2.5 Flash（入力 $0.30 / 出力 $2.50）より**大幅に高単価**。1 枚あたりの実コストは画像トークン数・出力長に依存する。**実測は未実施**だが、Claude の画像トークン換算と ADR-0010 の Gemini 実測プロファイル（単ページ・入力主体・出力 200〜500 トークン）から**概算**すると以下（`global` エンドポイント基準。**リージョン/マルチリージョン指定時は +10%**）。

  | モデル（`CLAUDE_MODEL`）  | 概算/枚          | 対 Gemini 2.5 Flash | 対 Document AI（$0.10 固定） |
  | ------------------------- | ---------------- | ------------------- | ---------------------------- |
  | `claude-opus-4-8`（既定） | 約 $0.015〜0.04  | 約 10〜20 倍        | 約 1/3〜1/6（安い）          |
  | `claude-sonnet-5`         | 約 $0.01〜0.02   | 約 6〜11 倍         | 約 1/5〜1/10                 |
  | `claude-haiku-4-5`        | 約 $0.004〜0.009 | 約 3〜5 倍          | 約 1/10〜1/25                |

  入力（画像）トークンが支配的なため、コスト削減は主に **①送信前の画像縮小** と **②`CLAUDE_MODEL` の下位化** が効く。多ページ PDF は 10 ページまで $0.10 固定の Document AI が有利な場合がある。**上記はあくまで概算**であり、本番採用前に代表サンプルで実トークン・実コストを測定する（下記「検証（PR-B）」）。
...
- **レイテンシー**: `claude-opus-4-8` は高精度な一方、応答時間は Gemini Flash より長くなり得る（thinking は無効運用だが上位モデルのため）。実応答時間は**未実測**で、本番採用前にサンプルで測定する（下記「検証（PR-B）」）。結果に応じて Cloud Functions のタイムアウト（gen2 デフォルト 60 秒）と `CLAUDE_TIMEOUT_MS`（既定 30 秒）の見直し、または `CLAUDE_MODEL` の下位化を検討する。
- **データレジデンシー・個人情報**: 本 ADR では既定を `global` エンドポイントとする（可用性優先・追加料金なし）。ただし `global` は**処理リージョン（データ所在）を保証しない**動的ルーティングである。領収書には店名・日付・金額など個人情報・取引情報が含まれるため、データ所在が要件（個人情報保護法・契約上）となった場合は `CLAUDE_LOCATION` を以下いずれかに切り替える（いずれも **global 比 +10% の料金プレミアム**）。
  - **マルチリージョン**: `us`（米国）/ `eu`（EU）のみ。**アジアのマルチリージョンは提供されていない**。
  - **リージョン指定**: 特定リージョン（例 `us-east1` / `europe-west1`）。Vertex 上の Claude は**リージョンごとに提供状況が異なり**、**東京 `asia-northeast1` で `claude-opus-4-8` が提供されている保証はない**（アジアでは `asia-southeast1` 等が候補になり得る）。対象リージョンでの提供を **Model Garden で確認**してから設定する。
  - データ所在要件が生じた時点で提供状況を確認し、`CLAUDE_LOCATION` を env で上書きする運用とする。
...
- **リージョン/モデルの可用性（新規制約）**: Gemini と異なり、Vertex 上の Claude は**リージョンごとに提供モデルが異なる**。`CLAUDE_LOCATION` で選ぶリージョン（または `global` エンドポイント）で `CLAUDE_MODEL`（既定 `claude-opus-4-8`）が提供されていることをデプロイ前に Model Garden で確認する。対象リージョンでモデルが未提供、またはモデル ID／リソースパスの指定が不正な場合、エンドポイント URL が解決できず「リソース未検出」（404 等）となる。
- **プロンプト管理**: 抽出品質はプロンプトに依存する。プロンプトは Gemini と同様に**バージョン管理下のコード内定数**として保持し、変更は PR/デプロイ単位で反映する。
- **API 仕様上の注意（受容リスク）**: `temperature` / `top_p` / `top_k` の可否や `thinking` の既定は**モデルに依存する**。既定の `claude-opus-4-8`（および opus-4.7 / Fable 5 等の新しめのモデル）ではこれらサンプリングパラメータを含めるとリクエストが拒否され（400）、`thinking` は省略で無効になる。Gemini 実装（`temperature:0` / `thinkingBudget:0`）からのコピー時にこれらを持ち込まないこと。`CLAUDE_MODEL` を古いモデルに切り替える場合は当該モデルの対応可否を確認する。数値表記揺れ（税込/税別・全角・カンマ）は infrastructure 層の正規化で吸収するが、**合計・税額・明細の整合検証は本 ADR のスコープでは行わない**（ADR-0011 と同様、application 層での整合検証は将来の余地として残す）。
...
- **検証（PR-B / 本番採用前）**: 上記コスト・レイテンシーは概算・未実測のため、実装 PR（PR-B）の検証時に代表サンプル数枚で以下を実測し、結果を記録する。必要なら既定 `CLAUDE_MODEL` を再検討する。
  - **トークン・コスト**: レスポンスの `usage.input_tokens` / `usage.output_tokens` から 1 枚あたりの実トークンと概算コストを算出（ADR-0010 の Gemini 実測と同粒度）。
  - **レイテンシー**: 実応答時間を計測し、`CLAUDE_TIMEOUT_MS` と Cloud Functions タイムアウトの妥当性を確認。
  - **抽出精度**: 手書き含むサンプルで Document AI / Gemini / Claude を比較し、エンジン選択の妥当性を確認。
