...
- **コスト**: 本構成（Gemini 2.5 Flash・単ページ領収書・thinking 無効）は、Document AI Expense Parser（**$0.10/ドキュメント固定**、単ページでも $0.10）より**大幅に安価**。単価は入力 $0.30 / 出力 $2.50（100万トークンあたり、2026-06 時点）。
  - **実測**（`input/` の手書き3枚・4032×3024px・`gemini-2.5-flash`・`thinkingBudget:0`、`usageMetadata` 実値）: 入力 **3,493 トークン**（画像込み・固定）、出力 **182〜462 トークン**、**平均 $0.00175/枚**（Document AI の**約 1/57**）。
  - **月額試算**: 1,000枚 ≈ **$1.75**（vs Document AI $100）、10,000枚 ≈ **$18**（vs $1,000）。
  - **注意**: 高解像度画像の送信・thinking 有効化・上位モデル（2.5 Pro）でコストは増加しうるため、送信前の画像縮小と `thinkingBudget:0` を推奨。**多ページPDF**は 10ページまで $0.10 の Document AI が有利。
