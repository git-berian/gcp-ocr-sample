...
- **2026-10-16**: 公式ドキュメント上の retirement 日（通知メールでは 2026-10-20。数日ずれるが 2026-10 中である点は同じ）
- **2026-10-20**: ELA（Extended Lifecycle Access）入り。**呼び出しは継続可能**で、価格も 2027-01-28 まで据え置き。この時点で API が停止するわけではない
- **2027-01-28**: ELA 標準価格が終了し**大幅値上げ**。リージョン提供状況も変わりうる。**実運用上の移行期限はこちら**
...
- `thinkingConfig: { thinkingBudget: 0 }` は**現状維持**する（Gemini 3 系でもそのまま通り、thinking は無効化される）。将来 `thinkingLevel` に置き換える場合は、**`thinkingBudget` と同時指定しない**こと（併用は 400 エラー）。置き換えは thinking 有効化が必要になった場合、または `thinkingBudget` が受理されなくなった場合に限り、本 ADR とは別に再測定したうえで行う
...
| モデル                               | 精度      | 入力Tok   | 出力Tok平均 | 単価 in/out（$/1M） | 入力$         | 出力$         | 平均 $/枚     | 現行比  | 平均レイテンシー |
| ------------------------------------ | --------- | --------- | ----------- | ------------------- | ------------- | ------------- | ------------- | ------- | ---------------- |
| `gemini-2.5-flash`（現行・廃止対象） | 24/25     | 3,520     | 237.4       | 0.30 / 2.50         | $0.001056     | $0.000593     | $0.001649     | —       | 3,279ms          |
| **`gemini-3.5-flash-lite`**          | **25/25** | **1,366** | **260.6**   | **0.30 / 2.50**     | **$0.000410** | **$0.000651** | **$0.001061** | **64%** | 3,051ms          |
| `gemini-3.1-flash-lite`              | 25/25     | 1,366     | 235.8       | 0.25 / 1.50         | $0.000342     | $0.000354     | $0.000695     | 42%     | 3,119ms          |
| `gemini-3.5-flash`                   | 25/25     | 1,366     | 262.6       | 1.50 / 9.00         | $0.002049     | $0.002363     | $0.004412     | 268%    | 2,962ms          |

`平均 $/枚 = 入力Tok × 入力単価 ÷ 1e6 + 出力Tok平均 × 出力単価 ÷ 1e6`。入力トークンは同一画像・同一プロンプトのため 5 枚とも固定値。thinking は全モデルで無効（`thoughtsTokenCount = 0`）のため thinking トークンの課金は発生していない。
...
- **コストは全候補が現行より安い**。主因は Gemini 3 系で同一画像の入力トークンが 3,520 → **1,366（61% 減）** になったこと。`gemini-3.5-flash-lite` は単価が現行と同じで、出力トークンはむしろ増えている（237.4 → 260.6）ため**出力コストは $0.000593 → $0.000651 と微増**しており、36% の削減はすべて入力側で生じている
- **レイテンシーは各条件 1 回のみの計測で、統計的な検定はしていない**。今回の 5 枚では平均 2.9〜3.3 秒の範囲に収まり、モデル間で大きな差は観測されなかった

### `gemini-3.5-flash-lite` を選ぶ理由

- **`gemini-3.1-flash-lite` は最安（現行の 42%）だが、既に廃止日 2027-05-07 が設定済み**で、Google 自身が移行先として `gemini-3.5-flash-lite` を案内している。公式ドキュメントは廃止日を「retire されうる最も早い日」と位置づけているため前倒しはされないが、採用すれば 1 年程度で再移行が必要になる見込みで、今回と同じ作業を繰り返す
...
- **`gemini-3.1-flash-lite` を採用**: 最安だが廃止日が設定済みで、1 年程度で再移行が必要になる見込み。コスト差（10,000枚で年間 $3.66）は再移行の作業コストに見合わない
